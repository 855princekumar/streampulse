#!/usr/bin/env python3
"""
Hybrid RTSP + MJPEG Stream Health Monitor — v4.3 (Baseline SLA-ready)
...
✅ NTP timestamps, timezone from config.yaml
✅ Hot reload config.yaml; creates tables for new streams
✅ /metrics endpoint: always fast (light metrics)
✅ Adds BASIC 30d SLA sample-based metrics (COUNT based, not time-duration based)
✅ Adds ts index per stream table for fast window queries
"""
...
BACKOFF_BASE = float(os.getenv("BACKOFF_BASE", "5.0"))
CFG_RELOAD_SEC = 3

# ✅ light SLA window config
SLA_WINDOW_DAYS = int(os.getenv("SLA_WINDOW_DAYS", "30"))
...
def read_cfg():
    if not CFG_PATH.exists():
        CFG_PATH.write_text(
            yaml.safe_dump({"heartbeat_seconds": 10, "timezone": "UTC", "streams": []}, sort_keys=False)
        )
...
def init_db(cfg):
    ...
    for s in cfg["streams"]:
        ...
        cur.execute("INSERT OR REPLACE INTO streams(name,url) VALUES(?,?)", (name, url))
        cur.execute(
        ...
        # ✅ Production-safe indexing for SLA / time-window queries
        # This is safe to run on every boot (IF NOT EXISTS)
        cur.execute(f"CREATE INDEX IF NOT EXISTS idx_{tn}_ts ON {tn}(ts)")
        cur.execute(f"CREATE INDEX IF NOT EXISTS idx_{tn}_status ON {tn}(status)")
...
async def worker_loop(lane: Lane, sched: Scheduler, session: aiohttp.ClientSession, wid: int):
    while True:
        ...
        # fallback to cv2 frame check after repeated failures
        if not ok and s.fails >= 3:
            ok2, msg2, lat2 = one_frame(s.url)
            if ok2:
                ok, msg, lat = ok2, msg2, lat2
...
def metrics():
    now_utc = ntp_utc_now()
    window_start = now_utc - timedelta(days=SLA_WINDOW_DAYS)
    ...
    # ✅ SLA sample system totals (light)
    system_ok_samples_30d = 0
    system_total_samples_30d = 0

    lines = []
    lines.append("# HELP stream_up Stream availability (1=up, 0=down)")
    lines.append("# TYPE stream_up gauge")
    lines.append("# HELP stream_latency_ms Last measured check latency in milliseconds")
    lines.append("# TYPE stream_latency_ms gauge")
    lines.append("# HELP stream_last_seen_age_seconds Seconds since last DB log entry for the stream")
    lines.append("# TYPE stream_last_seen_age_seconds gauge")
    lines.append("# HELP stream_uptime_streak_seconds Current continuous uptime streak duration (seconds). 0 if currently down.")
    lines.append("# TYPE stream_uptime_streak_seconds gauge")

    for n in names:
        tn = tname(n)
        label = _prom_label_escape(n)

        try:
            # last status
            cur.execute(f"SELECT ts,status,latency_ms FROM {tn} ORDER BY id DESC LIMIT 1")
            r = cur.fetchone()

            if not r:
                lines.append(f'stream_up{{stream="{label}"}} 0')
                lines.append(f'stream_latency_ms{{stream="{label}"}} 0')
                lines.append(f'stream_last_seen_age_seconds{{stream="{label}"}} 999999')
                lines.append(f'stream_uptime_streak_seconds{{stream="{label}"}} 0')
                fail_count += 1
                continue

            last_ts, last_status, last_lat = r
            is_up = 1 if last_status == "ok" else 0
            ...
            if dt_last:
                age_sec = int((now_utc - dt_last.astimezone(timezone.utc)).total_seconds())
                if age_sec < 0:
                    age_sec = 0

            # uptime streak (from last fail to now)
            uptime_streak = 0
            if is_up == 1:
                cur.execute(f"SELECT ts FROM {tn} WHERE status='fail' ORDER BY id DESC LIMIT 1")
                last_fail = cur.fetchone()

                if last_fail:
                    fail_ts = _parse_iso(last_fail[0])
                    if fail_ts:
                        uptime_streak = int((now_utc - fail_ts.astimezone(timezone.utc)).total_seconds())
                else:
                    # never failed => streak since first ever record
                    cur.execute(f"SELECT ts FROM {tn} ORDER BY id ASC LIMIT 1")
                    first = cur.fetchone()
                    if first:
                        first_ts = _parse_iso(first[0])
                        if first_ts:
                            uptime_streak = int((now_utc - first_ts.astimezone(timezone.utc)).total_seconds())

                if uptime_streak < 0:
                    uptime_streak = 0

            lines.append(f'stream_up{{stream="{label}"}} {is_up}')
            lines.append(f'stream_latency_ms{{stream="{label}"}} {int(last_lat) if last_lat else 0}')
            lines.append(f'stream_last_seen_age_seconds{{stream="{label}"}} {age_sec}')
            lines.append(f'stream_uptime_streak_seconds{{stream="{label}"}} {uptime_streak}')

            # ✅ lightweight SLA (sample count)
            cur.execute(
                f"SELECT COUNT(*) FROM {tn} WHERE ts >= ?",
                (window_start_iso,),
            )
            total_samples = int(cur.fetchone()[0] or 0)

            cur.execute(
                f"SELECT COUNT(*) FROM {tn} WHERE ts >= ? AND status='ok'",
                (window_start_iso,),
            )
            ok_samples = int(cur.fetchone()[0] or 0)

            system_total_samples_30d += total_samples
            system_ok_samples_30d += ok_samples
    ...
    total = len(names)
    system_up = 1 if (total > 0 and ok_count == total) else 0

    # ---- system summary ----
    lines.append("# HELP system_total_streams Total streams monitored")
    lines.append("# TYPE system_total_streams gauge")
    lines.append("# HELP system_ok_streams Streams currently up")
    lines.append("# TYPE system_ok_streams gauge")
    lines.append("# HELP system_fail_streams Streams currently down")
    lines.append("# TYPE system_fail_streams gauge")
    lines.append("# HELP system_up Overall system status (1=all streams up else 0)")
    lines.append("# TYPE system_up gauge")

    lines.append(f"system_total_streams {total}")
    lines.append(f"system_ok_streams {ok_count}")
    lines.append(f"system_fail_streams {fail_count}")
    lines.append(f"system_up {system_up}")

    # ✅ Basic 30d system SLA sample metrics (light)
    lines.append(f"# HELP system_ok_samples_{SLA_WINDOW_DAYS}d Total OK samples across all streams in window")
    lines.append(f"# TYPE system_ok_samples_{SLA_WINDOW_DAYS}d gauge")
    lines.append(f"# HELP system_total_samples_{SLA_WINDOW_DAYS}d Total samples across all streams in window")
    lines.append(f"# TYPE system_total_samples_{SLA_WINDOW_DAYS}d gauge")
    lines.append(f"# HELP system_sla_ratio_{SLA_WINDOW_DAYS}d Sample-based SLA ratio across all streams (0..1)")
    lines.append(f"# TYPE system_sla_ratio_{SLA_WINDOW_DAYS}d gauge")

    system_ratio = (system_ok_samples_30d / system_total_samples_30d) if system_total_samples_30d > 0 else 0.0

    lines.append(f"system_ok_samples_{SLA_WINDOW_DAYS}d {system_ok_samples_30d}")
    lines.append(f"system_total_samples_{SLA_WINDOW_DAYS}d {system_total_samples_30d}")
    lines.append(f"system_sla_ratio_{SLA_WINDOW_DAYS}d {system_ratio:.6f}")
...
def main():
    import threading
    threading.Thread(
        target=lambda: app.run(host="0.0.0.0", port=7000, debug=False, use_reloader=False),
        daemon=True,
    ).start()

    print("✅ monitor v4.3 running (status API + metrics on :7000)")

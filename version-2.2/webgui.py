...
<label>Enable MQTT Publishing:</label>
<input type="checkbox" name="mqtt_enabled" {{'checked' if cfg.get('mqtt',{}).get('enabled') else ''}}>
<br><br>

<label>Broker Host:</label>
<input type="text" name="mqtt_host" value="{{cfg.get('mqtt',{}).get('host','')}}">
<br><br>

<label>TCP Port:</label>
<input type="number" name="mqtt_port" value="{{cfg.get('mqtt',{}).get('port',1883)}}">
<br><br>

<label>Websocket Port:</label>
<input type="number" name="mqtt_ws_port" value="{{cfg.get('mqtt',{}).get('ws_port',8000)}}">
<br><br>

<label>TLS TCP Port:</label>
<input type="number" name="mqtt_tls_port" value="{{cfg.get('mqtt',{}).get('tls_port',8883)}}">
<br><br>

<label>TLS Websocket Port:</label>
<input type="number" name="mqtt_tls_ws_port" value="{{cfg.get('mqtt',{}).get('tls_ws_port',8884)}}">
<br><br>

<label>Use TLS:</label>
<input type="checkbox" name="mqtt_use_tls" {{'checked' if cfg.get('mqtt',{}).get('use_tls') else ''}}>
<br><br>

<label>Use Websocket:</label>
<input type="checkbox" name="mqtt_use_ws" {{'checked' if cfg.get('mqtt',{}).get('use_websocket') else ''}}>
<br><br>

<label>Username (optional):</label>
<input type="text" name="mqtt_username" value="{{cfg.get('mqtt',{}).get('username','')}}">
<br><br>

<label>Password (optional):</label>
<input type="password" name="mqtt_password" value="{{cfg.get('mqtt',{}).get('password','')}}">
<br><br>

<label>Topic Name:</label>
<input type="text" name="mqtt_topic" value="{{cfg.get('mqtt',{}).get('topic','stream_monitor/status')}}">
<br><br>

<label>Publish Interval (sec, min 5):</label>
<input type="number" name="mqtt_interval" min="5" value="{{cfg.get('mqtt',{}).get('interval_seconds',10)}}">
<br><br>

<label>Status API URL:</label>
<input type="text" name="mqtt_api_url" value="{{cfg.get('mqtt',{}).get('api_status_url','http://localhost:7000/api/status')}}">
<br><br>
...
def settings():
    ...
    if request.method == "POST":
        ...
        mqtt["enabled"] = ("mqtt_enabled" in request.form)
        mqtt["host"] = request.form.get("mqtt_host", mqtt.get("host"))
        mqtt["port"] = int(request.form.get("mqtt_port", mqtt.get("port", 1883)))
        mqtt["ws_port"] = int(request.form.get("mqtt_ws_port", mqtt.get("ws_port", 8000)))
        mqtt["tls_port"] = int(request.form.get("mqtt_tls_port", mqtt.get("tls_port", 8883)))
        mqtt["tls_ws_port"] = int(request.form.get("mqtt_tls_ws_port", mqtt.get("tls_ws_port", 8884)))
        mqtt["use_tls"] = ("mqtt_use_tls" in request.form)
        mqtt["use_websocket"] = ("mqtt_use_ws" in request.form)
        mqtt["username"] = request.form.get("mqtt_username") or None
        mqtt["password"] = request.form.get("mqtt_password") or None
        mqtt["topic"] = request.form.get("mqtt_topic", mqtt.get("topic", "stream_monitor/status"))
        mqtt["interval_seconds"] = max(5, int(request.form.get("mqtt_interval", mqtt.get("interval_seconds", 10))))
        mqtt["api_status_url"] = request.form.get("mqtt_api_url", mqtt.get("api_status_url"))

        cfg["mqtt"] = mqtt
        write_cfg(cfg)
        # --- END MQTT SETTINGS PATCH ---
